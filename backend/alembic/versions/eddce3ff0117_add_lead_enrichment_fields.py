...
def upgrade() -> None:
    ...
    op.add_column(
        "leads",
        sa.Column("scope_of_work", postgresql.ARRAY(sa.String()), nullable=True),
    )
    op.add_column(
        "leads", sa.Column("floor_plan_url", sa.String(length=500), nullable=True)
    )
    op.add_column(
        "leads", sa.Column("design_style", sa.String(length=100), nullable=True)
    )
    op.add_column("leads", sa.Column("possession_date", sa.Date(), nullable=True))
    op.add_column(
        "leads",
        sa.Column("site_visit_availability", sitevisitavailability, nullable=True),
    )
